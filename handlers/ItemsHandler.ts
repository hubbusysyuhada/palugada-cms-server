...
export default class {
  static async findAll(req: FastifyRequest<{ Querystring: { supplier_id?: string; sub_category_id?: string; keywords?: string; limit: number; offset: number; order_by?: string; } }>, rep: FastifyReply) {
    ...
    if (req.query.keywords) condition.push({ $or: [ { unique_id: { $like: `%${req.query.keywords}%` } }, { description: { $like: `%${req.query.keywords}%` } } ] })
